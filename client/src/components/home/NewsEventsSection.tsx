...
export default function NewsEventsSection() {
  ...
  const handleEventsPrev = () => {
    setCurrentEventsPage((prev) => (prev - 1 + totalEventsPages) % totalEventsPages);
  };
      ...
      <div className="container mx-auto px-4">
        <div className="flex justify-between items-end mb-12">
          <div>
            ...
            <p className="text-lg text-gray-300">
              Stay updated with the latest news and upcoming events from TCS
            </p>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          <div className="bg-gray-800 rounded-lg shadow-lg overflow-hidden">
            <div className="p-6">
              <h3 className="text-2xl font-bold text-white mb-4">Latest News</h3>
              
              <div className="space-y-6">
                {displayedNews.map((newsItem, index) => (
                  <div 
                    key={newsItem.id} 
                    className={`${index < displayedNews.length - 1 ? 'border-b border-gray-700 pb-6' : ''}`}
                  >
                    <div className="flex items-center mb-2">
                      <span className="text-sm text-gray-400">{newsItem.date}</span>
                      <span className="mx-2 text-gray-500">•</span>
                      <span className="text-sm text-[#00A0DF] font-medium">{newsItem.category}</span>
                    </div>
                    <h4 className="text-lg font-bold text-white mb-2">{newsItem.title}</h4>
                    <p className="text-gray-300">{newsItem.description}</p>
                    <a href={newsItem.link} className="text-[#00A0DF] font-medium mt-2 inline-block hover:underline">
                      Read More
                    </a>
                  </div>
                ))}
              ...
              <a href="#" className="text-[#00A0DF] font-medium mt-6 inline-block">
            ...
            <div className="p-6">
              ...
              <div className="space-y-6">
                {displayedEvents.map((event, index) => (
                  <div 
                    key={event.id} 
                    className={`flex ${index < displayedEvents.length - 1 ? 'border-b border-gray-700 pb-6' : ''}`}
                  >
                    ...
                    <div>
                      ...
                      <p className="text-sm text-gray-400">
                        <i className={`fas fa-${event.isOnline ? 'globe' : 'map-marker-alt'} mr-1`}></i> {event.location} | <i className="fas fa-clock mr-1"></i> {event.time}
                      </p>
                      <a href={event.link} className="text-[#00A0DF] font-medium mt-2 inline-block hover:underline">
                        Register Now
                      </a>
                    </div>
                  </div>
